...
export default function SolicitudesList() {
  ...
  const load = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const { data, count: c, error: e } = await listSolicitudesAdmin({ page, pageSize: PAGE_SIZE_SOLICITUDES, search });
      if (e) throw e;
      setRows(data);
      setCount(c);
    } catch (e: any) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-4xl font-bold tracking-tight">Solicitudes</h1>
      </div>
